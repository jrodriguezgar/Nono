...
| Option | Description |
|--------|-------------|
| `--provider`, `-p` | Provider: google, openai, perplexity, deepseek, xai, groq, cerebras, nvidia, huggingface, openrouter, foundry, vercel, ollama |
| `--model`, `-m` | Model name |
...
The CLI integrates with the [configuration module](../config/README_config.md) for centralized settings management.
...
The CLI includes subcommands for managing Nono projects:

### `nono init` — Create a new project

```bash
# Create in current directory
nono init --name "My Project"

# Create in a subdirectory with all options
nono init my-project -n "My Project" -d "Text analysis pipeline" -p openai
```

| Argument | Description | Default |
|----------|-------------|---------|
| `path` | Directory to create (positional) | `.` |
| `--name`, `-n` | Project name | directory name |
| `--description`, `-d` | One-line description | `""` |
| `--provider`, `-p` | Default LLM provider | `google` |

Creates: `nono.toml`, `config.toml`, `skills/`, `prompts/`, `templates/`, `workflows/`.

### `nono project` / `nono proj` — Show project info

```bash
# Auto-detect project (walks up from cwd)
nono project

# Explicit path
nono project --path ./my-project
...
| Argument | Description | Default |
|----------|-------------|---------|
| `--path` | Explicit project directory | auto-detect |

Displays: name, version, description, provider, resource counts, skill details.

📖 See also: [Projects Documentation](../README_projects.md)

📖 See also: [Configuration Documentation](../config/README_config.md)
